    ...
    try:
        ...
        class ChatOpenAI:
            ...
            def invoke(self, messages, config=None):
                return type('MockResponse', (), {"content": "Respuesta simulada del LLM"})()
    ...
    def _setup_llm(self):
        ...
        try:
            if settings.openai_api_key:
                ...
                self.logger.info(f"✅ LLM configurado con OpenAI - Modelo: {settings.openai_model}")
                return llm
        except Exception as e:
            self.logger.error(f"Error configurando OpenAI: {e}")
        
        # Fallback a mock
        self.logger.warning("⚠️ Usando LLM mock para desarrollo")
        class MockLLM:
            def __init__(self):
                self.model = "mock"
                self.temperature = 0
            def invoke(self, messages, config=None):
                return type('MockResponse', (), {"content": "Respuesta simulada del LLM mock"})()
        
        return MockLLM()

    def process_request(self, input_data: Any, session_id: Optional[str] = None, config: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Guardar configuración (para Langfuse)
            if config:
                self.memory["processing_context"]["current_config"] = config
